...
class DiagnoseResponse(BaseModel):
    ...
    confirm_reason_code: str | None = None
    confirm_reason_text: str | None = None
    recommended_action: str | None = None
    confirm_ui_mode: str | None = None
...
def _normalize_reason_tokens(value: Any) -> list[str]:
    if isinstance(value, list):
...
def _reason_contains(reason_tokens: list[str], target: str) -> bool:
...
def build_confirm_explanation_v2(
    *,
    ...
    if not need_confirm:
        return {
            "confirm_reason_code": None,
            "confirm_reason_text": None,
            "recommended_action": None,
            "confirm_ui_mode": None,
            "confirm_fields": [],
            "confirm_message": None,
        }

    reason_tokens = _normalize_reason_tokens(fallback_reason)
    fusion_case_text = str(fusion_case or "").strip().lower()
    supplement_mode_text = str(supplement_mode or "").strip().lower()
    image_ok = image_reliable is True
    ...
    if image_only_quality_case:
    ...
    elif _reason_contains(reason_tokens, "low_margin"):
        ...
        or supplement_mode_text == "image_and_text"
    ):
        reason_code = "BOTH_IMAGE_AND_TEXT_WEAK"
        reason_text = "当前图片和症状描述都不足，无法稳定判断"
        ...
        action = "supplement_symptoms"
        ui_mode = "text"
        fields = ["symptoms"]
    elif _reason_contains(reason_tokens, "no_image_parsed"):
        reason_code = "NO_IMAGE_PARSED"
        reason_text = "图片解析失败，请重新上传清晰图片"
        action = "reupload_image"
        ui_mode = "image"
        fields = ["image"]
    elif _reason_contains(reason_tokens, "no_effective_text_evidence"):
        reason_code = "NO_EFFECTIVE_TEXT_EVIDENCE"
        reason_text = "症状描述未提供有效证据，请补充关键症状"
        action = "supplement_symptoms"
        ui_mode = "text"
        fields = ["symptoms"]
    else:
    ...
    return {
        ...
        "confirm_reason_text": reason_text,
...
def build_start_confirm_explanation(
    *,
    need_confirm: bool,
    low_conf: bool,
    low_margin: bool,
    ...
    if low_conf and low_margin:
        ...
        supplement_mode = "text_only" if symptoms_list else "image_and_text"
    return build_confirm_explanation(
        need_confirm=need_confirm,
        fusion_case="both_weak" if (low_conf and low_margin) else "normal",
        image_reliable=not low_conf,
        text_reliable=bool(symptoms_list),
        supplement_mode=supplement_mode,
        fallback_reason=fallback_reason,
        follow_up_questions=symptoms_list[:3],
    )
...
def _latest_case_event_by_trace(trace_id: str) -> dict[str, Any]:
    def _has_diagnosis_context(event_like: dict[str, Any]) -> bool:
        if not isinstance(event_like, dict):
            return False
        image_result_like = _safe_record(event_like.get("image_result"))
        image_diag_like = _safe_record(event_like.get("image_diagnosis"))
        diagnosis_evidence_like = _safe_record(event_like.get("diagnosis_evidence"))
        return bool(
            event_like.get("final_disease")
            or event_like.get("final_confidence") is not None
            or _as_clean_list(event_like.get("fusion_top3"))
            or _as_clean_list(event_like.get("text_top3"))
    ...
    if isinstance(event, dict) and _has_diagnosis_context(event):
        return event

    normalized_trace_id = str(trace_id or "").strip()
    if not normalized_trace_id:
        return event if isinstance(event, dict) else {}
    for item in list_events(5000):
        if not isinstance(item, dict):
            continue
        if str(item.get("trace_id") or "").strip() != normalized_trace_id:
            continue
        if _has_diagnosis_context(item):
            return item
    return event if isinstance(event, dict) else {}
...
def _build_image_diagnosis_from_event(event: dict[str, Any]) -> dict[str, Any]:
    ...
    image_diagnosis = _safe_record(event.get("image_diagnosis"))
    top3 = _normalize_top3_candidates(image_result.get("top3"))
    if not top3:
        top3 = _normalize_top3_candidates(image_diagnosis.get("top3"))
    ...
    if not top1_disease:
        top1_disease = str(_safe_record(image_diagnosis.get("top1")).get("disease") or "").strip()
    if top1_confidence is None:
        top1_confidence = _safe_float(_safe_record(image_diagnosis.get("top1")).get("confidence"))
    elif top1_confidence <= 0:
        image_diag_conf = _safe_float(_safe_record(image_diagnosis.get("top1")).get("confidence"))
        if image_diag_conf is not None and image_diag_conf > 0:
            top1_confidence = image_diag_conf
...
def _resolve_confirm_choice_confidence(choice: str, previous_case_event: dict[str, Any]) -> float | None:
    ...
    for candidates in (
        ...
        _normalize_top3_candidates(_safe_record(previous_case_event.get("image_diagnosis")).get("top3")),
        ...
        _normalize_top3_candidates(diagnosis_evidence.get("text_top3")),
    ):
...
def _inherit_previous_diagnosis_context(
    ...
    *,
    ...
    diagnosis_evidence_record = diagnosis_evidence if isinstance(diagnosis_evidence, dict) else {}
    if not text_top3:
        text_top3 = _normalize_top3_candidates(diagnosis_evidence_record.get("text_top3"))
    if not fusion_top3:
        fusion_top3 = _normalize_top3_candidates(diagnosis_evidence_record.get("fusion_top3"))
    resolved_confidence = _resolve_confirm_choice_confidence(choice, previous_case_event)
    if resolved_confidence is None:
        resolved_confidence = _safe_float(state.get("final_confidence"))
    if resolved_confidence is None:
        resolved_confidence = _safe_float(previous_case_event.get("final_confidence"))
    ...
    if isinstance(diagnosis_evidence, dict) and diagnosis_evidence:
    ...
    merged_image_result = dict(previous_image_result)
    if not _as_clean_list(merged_image_result.get("top3")) and image_diagnosis.get("top3"):
        merged_image_result["top3"] = [
            {"disease": name, "prob": float(prob), "prob_pct": round(float(prob) * 100, 2)}
            for name, prob in list(image_diagnosis.get("top3") or [])
        ]
    if not merged_image_result.get("disease"):
        merged_image_result["disease"] = _safe_record(image_diagnosis.get("top1")).get("disease")
    if _safe_float(merged_image_result.get("confidence")) is None:
        top1_conf = _safe_float(_safe_record(image_diagnosis.get("top1")).get("confidence"))
        if top1_conf is not None:
            merged_image_result["confidence"] = top1_conf
            merged_image_result["confidence_pct"] = round(top1_conf * 100, 2)
    if image_diagnosis:
        state["image_diagnosis"] = image_diagnosis
    if merged_image_result:
        state["image_result"] = merged_image_result
...
def _lightweight_diagnosis_payload(
    *,
    ...
    need_confirm: bool,
    confirm_explanation: dict[str, Any] | None,
    symptoms_list: list[str],
) -> dict[str, Any]:
    status = "waiting_for_supplement" if need_confirm else "diagnosis_completed"
    explain = confirm_explanation if isinstance(confirm_explanation, dict) else {}
    return {
        "trace_id": trace_id,
        "image_id": image_id,
        ...
        "need_confirm": need_confirm,
        "confirm_message": explain.get("confirm_message") if need_confirm else None,
        "confirm_reason_code": explain.get("confirm_reason_code") if need_confirm else None,
        "confirm_reason_text": explain.get("confirm_reason_text") if need_confirm else None,
        "recommended_action": explain.get("recommended_action") if need_confirm else None,
        "confirm_ui_mode": explain.get("confirm_ui_mode") if need_confirm else None,
        "confirm_fields": list(explain.get("confirm_fields") or []) if need_confirm else [],
...
async def diagnose_image_start(
    ...
    confirm_explanation = build_start_confirm_explanation(
        need_confirm=need_confirm,
        low_conf=low_conf,
    ...
    emit_node_event(trace_id, node="DiagnosisCompleted", status="end", message="诊断阶段完成", payload={
        ...
        "need_confirm": need_confirm,
        "confirm_reason_code": confirm_explanation.get("confirm_reason_code"),
        "confirm_ui_mode": confirm_explanation.get("confirm_ui_mode"),
        "confirm_fields": confirm_explanation.get("confirm_fields"),
    })
...
def _diagnose_confirm_core(request: Request, payload: dict) -> dict:
    request_started = time.perf_counter()
    trace_id = payload.get("trace_id")
    previous_trace_id = payload.get("previous_trace_id")
    image_id = payload.get("image_id")
    crop_type = payload.get("crop_type") or "番茄"
    symptoms = _normalize_symptoms_input(payload.get("symptoms"))
    ...
    inherited_image_result = _safe_record(state.get("image_result"))
    image_top1 = image_diagnosis.get("top1") or {}
    top3 = image_diagnosis.get("top3") or []
    if not top3:
        top3 = _normalize_top3_candidates(inherited_image_result.get("top3"))
    if not image_top1:
        image_top1 = {
            "disease": inherited_image_result.get("disease"),
            "confidence": _safe_float(inherited_image_result.get("confidence")),
        }
    image_result = {
    ...
    response_payload = {
        ...
        **_build_image_refs(image_id),
        ...
        "confirm_message": confirm_message,
        "confirm_reason_code": confirm_explanation.get("confirm_reason_code"),
        "confirm_reason_text": confirm_explanation.get("confirm_reason_text"),
        "recommended_action": confirm_explanation.get("recommended_action"),
        "confirm_ui_mode": confirm_explanation.get("confirm_ui_mode"),
        "confirm_fields": list(confirm_explanation.get("confirm_fields") or []),
        "treatment": None if confirm_status == "pending_expert_review" else {
            "plan": state.get("treatment_plan"),
            "prevention": state.get("prevention_advice"),
        },
...
async def diagnose_retry(
    request: Request,
    trace_id: str | None = Form(None),
    previous_trace_id: str | None = Form(None),
    ...
    if file is not None and file.filename:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"上传文件不是有效图片: {exc}") from exc
        resolved_image_id = f"{uuid.uuid4().hex}{suffix}"
        saved_path = UPLOAD_DIR / resolved_image_id
        saved_path.write_bytes(data)
        cleanup_old_uploads()
    ...
    payload = {
        ...
        "crop_type": crop_type,
        "symptoms": _normalize_symptoms_input(symptoms),
        "growth_stage": growth_stage,
        "model_id": model_id,
        "choice": choice,
        "farmer_id": farmer_id,
