...
export type TraceSourceHint = 'start' | 'continue' | 'replay' | 'confirm' | 'unknown';
export type TraceProtocol = 'workflow_snapshot' | 'compact_replay' | 'unknown';
...
const parseSourceHint = (source: unknown): TraceSourceHint => {
...
const sourcePriority = (sourceHint: TraceSourceHint): number => {
...
export const detectTraceProtocol = (rawEvent: unknown): TraceProtocol => {
...
const dedupeWithinBucket = (events: NormalizedTraceEvent[]): NormalizedTraceEvent[] => {
...
export const splitTraceEventsByProtocol = (events: NormalizedTraceEvent[]): {
  workflowSnapshotEvents: NormalizedTraceEvent[];
  compactReplayEvents: NormalizedTraceEvent[];
  unknownEvents: NormalizedTraceEvent[];
} => {
  const workflowSnapshotEvents: NormalizedTraceEvent[] = [];
  const compactReplayEvents: NormalizedTraceEvent[] = [];
  const unknownEvents: NormalizedTraceEvent[] = [];
