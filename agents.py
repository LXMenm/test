...
def diagnosis_agent(state: CropDiseaseState) -> CropDiseaseState:
    ...
    if hasattr(diagnosis_engine, "build_diagnosis_evidence"):
        ...
        diagnosis_evidence = {
            ...
            "text_top3": text_top3,
            "image_top3_adjusted": list((fusion_meta.get("pre_fusion_top3_adjusted", {}) or {}).get("image", []) if isinstance(fusion_meta, dict) else []),
            "text_top3_adjusted": list((fusion_meta.get("pre_fusion_top3_adjusted", {}) or {}).get("text", []) if isinstance(fusion_meta, dict) else []),
...
请输出1-2条与位置/设施/偏好有关的诊断风险提醒。"""
    ...
    if image_path and state.get("image_diagnosis") and isinstance(fusion_meta, dict):
        adjusted_image_top3 = list((fusion_meta.get("pre_fusion_top3_adjusted", {}) or {}).get("image", []))
        if adjusted_image_top3:
            state["image_diagnosis"]["top3_adjusted"] = adjusted_image_top3
