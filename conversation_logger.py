...
def log_conversation(state: Dict[str, Any], log_dir: str = DEFAULT_LOG_DIR) -> str:
    ...
    if profile:
        entry["profile_schema_version"] = profile.get("profile_schema_version")
        entry["profile_updated_at"] = profile.get("profile_updated_at")
        from personalization.profile_store import compute_profile_hash
        entry["profile_hash"] = compute_profile_hash(profile)
