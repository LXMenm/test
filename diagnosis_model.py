...
class DiseaseDiagnosisEngine:
    ...
    def _map_label_en(self, label_cn: str) -> str:
        """将中文标签映射回英文标签"""
        if not hasattr(self, '_label_map_cn_reverse'):
            self._label_map_cn_reverse = {v: k for k, v in self.label_map_cn.items()}
        return self._label_map_cn_reverse.get(label_cn, label_cn)
    ...
    @staticmethod
    def _prior_hint_score(raw_prior: float) -> float:
        """将先验 raw 概率映射为展示用“偏好分”，避免被误读为后验确定度。"""
        value = max(float(raw_prior or 0.0), 0.0)
        return value / (value + 0.5) if value > 0 else 0.0

    def _prior_hint_topk(self, prior_probs: Dict[str, float], k: int = 3) -> List[Tuple[str, float]]:
        raw_topk = self._topk(prior_probs, k)
        return [(name, self._prior_hint_score(raw_score)) for name, raw_score in raw_topk]
    ...
    def _apply_confusion_adjustment_to_probs(
        self,
        probs: Dict[str, float],
        original_class_cn: str,
        adjusted_class_cn: str,
        final_confidence: float,
    ) -> Dict[str, float]:
        if not probs:
            return {}
        new_probs = {k: max(float(v), 0.0) for k, v in probs.items()}
        original_class_cn = str(original_class_cn or "").strip()
        adjusted_class_cn = str(adjusted_class_cn or "").strip()
        if not original_class_cn or not adjusted_class_cn:
            return self._normalized(new_probs)
        if original_class_cn not in new_probs:
            new_probs[original_class_cn] = 0.0
        if adjusted_class_cn not in new_probs:
            new_probs[adjusted_class_cn] = 0.0
        bounded_conf = max(0.0, min(float(final_confidence or 0.0), 1.0))
        new_probs[original_class_cn] = bounded_conf
        if adjusted_class_cn != original_class_cn:
            old_adjusted = float(new_probs.get(adjusted_class_cn, 0.0))
            desired_adjusted = max(old_adjusted, bounded_conf)
            transfer_amount = max(0.0, desired_adjusted - old_adjusted)
            if transfer_amount > 0:
                donor_classes = [cls for cls in new_probs if cls != adjusted_class_cn and new_probs.get(cls, 0.0) > 0]
                donor_total = sum(new_probs.get(cls, 0.0) for cls in donor_classes)
                if donor_total > 0:
                    for cls in donor_classes:
                        reduction = transfer_amount * (new_probs[cls] / donor_total)
                        new_probs[cls] = max(0.0, new_probs[cls] - reduction)
            new_probs[adjusted_class_cn] = desired_adjusted
        return self._normalized(new_probs)
    ...
    def fuse_multimodal_probs(self, image_probs: Dict[str, float], text_probs: Dict[str, float], prior_probs: Dict[str, float], image_confidence: float = 0.0, text_confidence: float = 0.0, text_evidence_active: Optional[bool] = None, normalized_symptoms: Optional[List[str]] = None, image_quality_flags: Optional[List[str]] = None, image_quality_hint: Optional[str] = None) -> Tuple[Dict[str, float], Dict[str, object]]:
        ...
        prior_hint_top3 = self._prior_hint_topk(prior_probs, 3)
        image_top3_raw = list(image_top3)
        text_top3_raw = list(text_top3)
        ...
        text_probs_for_fusion = text_probs.copy() if reliable_text else {}
        image_top1 = image_top3[0][0] if image_top3 else None
        text_top1 = text_top3[0][0] if text_top3 else None
        
        # 处理易混淆类别
        image_confusion_result = None
        text_confusion_result = None
        
        # 用于融合的图像概率分布
        image_probs_for_fusion = image_probs.copy()
        
        if image_top1:
            # 将中文标签转换为英文标签进行易混淆处理
            image_top1_en = self._map_label_en(image_top1)
            image_top3_en = [(self._map_label_en(cls), conf) for cls, conf in image_top3[1:]]
            image_confusion_result = handle_confusing_cases(
                image_top1_en,
                image_top1_conf,
                normalized_symptoms or [],
                image_top3_en
            )
            if image_confusion_result and image_confusion_result['is_adjusted']:
                # 将调整后的英文标签转换回中文
                adjusted_class_en = image_confusion_result['final_class']
                image_top1 = self._map_label_cn(adjusted_class_en)
                image_top1_conf = image_confusion_result['final_confidence']
                
                # 创建调整后的图像概率分布
                original_class_cn = self._map_label_cn(image_confusion_result['original_class'])
                adjusted_class_cn = image_top1
                if adjusted_class_cn not in image_probs_for_fusion:
                    image_probs_for_fusion[adjusted_class_cn] = 0.0
                image_probs_for_fusion = self._apply_confusion_adjustment_to_probs(
                    image_probs_for_fusion,
                    original_class_cn=original_class_cn,
                    adjusted_class_cn=adjusted_class_cn,
                    final_confidence=image_top1_conf,
                )
                image_top3 = self._topk(image_probs_for_fusion, 3)
                image_top1 = image_top3[0][0] if image_top3 else image_top1
                image_top1_conf = float(image_top3[0][1]) if image_top3 else float(image_top1_conf)
                image_top2_conf = float(image_top3[1][1]) if len(image_top3) > 1 else 0.0
                image_margin = image_top1_conf - image_top2_conf
                reliable_image = bool(image_top3 and image_top1_conf >= image_top1_threshold and image_margin >= image_margin_threshold)
        ...
        if text_top1:
            # 将中文标签转换为英文标签进行易混淆处理
            text_top1_en = self._map_label_en(text_top1)
            text_top3_en = [(self._map_label_en(cls), conf) for cls, conf in text_top3[1:]]
            ...
            if text_confusion_result and text_confusion_result['is_adjusted']:
                # 将调整后的英文标签转换回中文
                adjusted_class_en = text_confusion_result['final_class']
                text_top1 = self._map_label_cn(adjusted_class_en)
                text_top1_conf = text_confusion_result['final_confidence']
                original_class_cn = self._map_label_cn(text_confusion_result['original_class'])
                adjusted_class_cn = text_top1
                if adjusted_class_cn not in text_probs_for_fusion:
                    text_probs_for_fusion[adjusted_class_cn] = 0.0
                text_probs_for_fusion = self._apply_confusion_adjustment_to_probs(
                    text_probs_for_fusion,
                    original_class_cn=original_class_cn,
                    adjusted_class_cn=adjusted_class_cn,
                    final_confidence=text_top1_conf,
                )
                text_top3 = self._topk(text_probs_for_fusion, 3)
                text_top1 = text_top3[0][0] if text_top3 else text_top1
                text_top1_conf = float(text_top3[0][1]) if text_top3 else float(text_top1_conf)
                text_top2_conf = float(text_top3[1][1]) if len(text_top3) > 1 else 0.0
                text_margin = text_top1_conf - text_top2_conf
                reliable_text = bool(text_top3 and text_top1_conf >= text_top1_threshold and text_margin >= text_margin_threshold)
        ...
        # 检查是否触发了易混淆类别调整
        image_adjusted = image_confusion_result and image_confusion_result.get("is_adjusted", False)
        text_adjusted = text_confusion_result and text_confusion_result.get("is_adjusted", False)
        confusion_adjustment_active = image_adjusted or text_adjusted
        ...
        if has_image and has_text:
            if reliable_image and reliable_text and conflict:
                ...
                # 如果图像被调整（易混淆类别），降低图像权重
                if image_adjusted:
                    base_weights = {"image": 0.35, "text": 0.65, "prior": 0.0}
                    confidence_drop_reason = "image_adjusted_for_confusion"
                else:
                ...
                # 如果图像被调整，降低图像权重，提高文本权重
                if image_adjusted:
                    base_weights = {"image": 0.40, "text": 0.60, "prior": 0.0}
                else:
                ...
                # 如果图像被调整，不要完全依赖图像
                if image_adjusted:
                    base_weights = {"image": 0.6, "text": 0.4, "prior": 0.0}
                else:
        ...
        adjusted_pre_fusion_top3 = {
            "image": self._topk(image_probs_for_fusion, 3),
            "text": self._topk(text_probs_for_fusion, 3),
            "prior": prior_top3,
        }
        keys = set(image_probs_for_fusion) | set(text_probs_for_fusion) | set(prior_probs)
        if not keys:
            meta = {"fuse_version": FUSE_MULTIMODAL_VERSION, "has_image": has_image, "has_text": has_text, "has_prior": has_prior, **reliability_summary, "image_top1_conf": image_top1_conf, "text_top1_conf": text_top1_conf, "image_margin": image_margin, "text_margin": text_margin, "fusion_case": fusion_case, "normalized_weights": normalized_weights, "pre_fusion_top1": {"image": image_top3[:1], "text": text_top3[:1], "prior": prior_top3[:1]}, "pre_fusion_top3": adjusted_pre_fusion_top3, "pre_fusion_top3_raw": {"image": image_top3_raw, "text": text_top3_raw, "prior": prior_top3}, "pre_fusion_top3_hint": {"prior": prior_hint_top3}, "pre_fusion_top3_adjusted": adjusted_pre_fusion_top3, "post_fusion_top3": [], "confidence_drop_reason": confidence_drop_reason, "modality_conflict_flag": conflict, "weak_conflict_candidate": weak_conflict_candidate, "strong_text_evidence": strong_text_evidence, "image_downgraded_on_conflict": image_downgraded_on_conflict, "image_confusion_result": image_confusion_result, "text_confusion_result": text_confusion_result, "insufficient_evidence": True, "degraded_reason": "no_active_evidence"}
            return {}, meta
        fused = {}
        for key in keys:
        ...
        meta = {"fuse_version": FUSE_MULTIMODAL_VERSION, "has_image": has_image, "has_text": has_text, "has_prior": has_prior, **reliability_summary, "image_top1_conf": image_top1_conf, "text_top1_conf": text_top1_conf, "image_margin": image_margin, "text_margin": text_margin, "fusion_case": fusion_case, "normalized_weights": normalized_weights, "pre_fusion_top1": {"image": image_top3[:1], "text": text_top3[:1], "prior": prior_top3[:1]}, "pre_fusion_top3": adjusted_pre_fusion_top3, "pre_fusion_top3_raw": {"image": image_top3_raw, "text": text_top3_raw, "prior": prior_top3}, "pre_fusion_top3_hint": {"prior": prior_hint_top3}, "pre_fusion_top3_adjusted": adjusted_pre_fusion_top3, "post_fusion_top3": self._topk(fused, 3), "confidence_drop_reason": confidence_drop_reason, "modality_conflict_flag": conflict, "weak_conflict_candidate": weak_conflict_candidate, "strong_text_evidence": strong_text_evidence, "image_downgraded_on_conflict": image_downgraded_on_conflict, "image_confusion_result": image_confusion_result, "text_confusion_result": text_confusion_result}
        return fused, meta
    ...
    def build_diagnosis_evidence(
        ...
        final_confidence: float,
        final_source: str,
        include_prior_raw_debug: bool = False,
    ) -> Dict[str, object]:
        image_top3 = self._topk(image_probs, 3)
        text_top3 = self._topk(text_probs, 3)
        prior_top3_raw = self._topk(prior_probs, 3)
        prior_top3 = self._prior_hint_topk(prior_probs, 3)
        fusion_top3 = self._topk(fusion_probs, 3)
        fusion_meta_payload = dict(fusion_meta) if isinstance(fusion_meta, dict) else {}
        if not include_prior_raw_debug and isinstance(fusion_meta_payload.get("pre_fusion_top3_raw"), dict):
            raw_map = dict(fusion_meta_payload.get("pre_fusion_top3_raw") or {})
            raw_map.pop("prior", None)
            if raw_map:
                fusion_meta_payload["pre_fusion_top3_raw"] = raw_map
            else:
                fusion_meta_payload.pop("pre_fusion_top3_raw", None)
        ...
        if prior_top3:
            detailed_reason += f"先验偏好top1={prior_top3[0][0]}(hint={prior_top3[0][1]:.2f})；"
        ...
        evidence = {
            ...
            "prior_semantics": "hint_score_preference",
            "fusion_top3": fusion_top3,
            "weights": fusion_meta_payload.get("normalized_weights") if isinstance(fusion_meta_payload, dict) else {},
            "fusion_meta": fusion_meta_payload,
            "modality_conflict_flag": modality_conflict_flag,
            "image_reliable": bool(fusion_meta_payload.get("image_reliable")) if isinstance(fusion_meta_payload, dict) else False,
            "text_reliable": bool(fusion_meta_payload.get("text_reliable")) if isinstance(fusion_meta_payload, dict) else False,
            "reliability_issue_types": list(fusion_meta_payload.get("reliability_issue_types") or []) if isinstance(fusion_meta_payload, dict) else [],
            "supplement_mode": str(fusion_meta_payload.get("supplement_mode") or "none") if isinstance(fusion_meta_payload, dict) else "none",
        ...
        if include_prior_raw_debug:
            evidence["debug_prior_top3_raw"] = prior_top3_raw
            evidence["debug_prior_semantics"] = "raw_probability_debug_only"
        return evidence
